...
- Phase: **0 — Foundation** (in progress — steps 0.1–0.7 of 0.15 done, see plan in chat history)
  ...
  - Next up: 0.8 Neon Postgres + Prisma (minimal `User` model + migration).
- Last green-light check: none yet (Prisma doesn't exist yet — added in step 0.8). Currently green: `npm run typecheck`, `npm run lint`, `npm run test`, `npm run test:e2e`, `npm run build`.
- Open blockers: none
- Last commit note: `test: add Playwright e2e setup, homepage smoke test` (PR #6, squash-merged by Rad)

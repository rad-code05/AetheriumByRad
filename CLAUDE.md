...
- Phase: **0 — Foundation** (in progress — steps 0.1–0.14 of 0.15 done, see plan in chat history)
  ...
  - 0.14 GitHub Actions CI: `.github/workflows/ci.yml` runs on every PR + push to `main` — checkout, Node 22 setup, `npm ci`, Playwright Chromium install, then typecheck/lint/unit-tests/build/e2e-tests in sequence, plus a Playwright HTML report uploaded as an artifact on any failure (`if: always()`). Required 5 real secrets added to GitHub (`gh secret set ...`, run by Rad so real values never touched the chat): `DATABASE_URL`, `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, `CLERK_SECRET_KEY`, `CLERK_PUBLISHABLE_KEY`, `E2E_CLERK_USER_EMAIL`. The 4 non-secret Clerk routing vars are hardcoded directly in the workflow YAML. **Known simplification**: CI reuses the real Neon dev database rather than a dedicated CI/test database (Neon branching would be the cleaner fix later) — also why `npx prisma migrate status` is deliberately left out of CI for now (it wouldn't tell us anything new against a shared, always-in-sync dev DB). Claude wrote the YAML (config, not app code). Verified: PR #23's Actions run passed all steps for real on GitHub's own servers. Shipped via PR #23 (squash-merged).
  - Next up: 0.15 Deploy to Vercel + full Phase 0 green-light check (the final step — closes out Phase 0).
- Last green-light check: none yet (`npx prisma migrate status` not yet re-verified post-merge; CI runs everything else but not that command — see step 0.14 note). Currently green (locally and now also in CI): `npm run typecheck`, `npm run lint`, `npm run test`, `npm run test:e2e` (covers all 3 Phase 0 DoD e2e requirements), `npm run build`.
- Open blockers:
  - **Next.js 16 `middleware.ts` → `proxy.ts` rename** — still not investigated. `middleware.ts` still works, low urgency, but should be researched before Phase 0 closes out (step 0.15).
  - **Recurring `.next` cache EPERM errors on Windows** — seen in steps 0.11 and 0.12 (local dev only, not seen in CI since that's a fresh Linux machine each run). Root cause still not pinned down (confirmed not OneDrive; likely antivirus real-time scanning or similar). Worth a permanent fix before it causes a real failure, but not blocking.
  - **CI shares the real Neon dev database** rather than a dedicated CI database (see step 0.14) — low risk at current scale, but worth a proper fix (Neon branching) before this matters more.
- Last commit note: `ci: add GitHub Actions workflow for lint/typecheck/tests/build` (PR #23, squash-merged)

### Error Log addendum
- **`.next` cache EPERM rename errors on Windows** · Phase 0, step 0.11 · resolved. Symptom: `/sign-in?redirect_url=...` returned 500 after `auth.protect()` correctly redirected there from `/dashboard`. Exact error (repeated for several files): `Error: EPERM: operation not permitted, rename 'C:\...\.next\dev\server\*.js.tmp.xxx' -> 'C:\...\.next\dev\server\*.js'`. Root cause: something on Windows (antivirus scanning, file indexing, or similar — confirmed NOT OneDrive) was locking Next.js/Turbopack's build-cache files while it tried to rename them into place; unrelated to the actual `dashboard`/`sign-in` code, which was correct. Ruled out a stray duplicate dev-server process first (checked via `tasklist`/`Get-CimInstance Win32_Process` — only one legitimate `npm run dev` process tree was running). Fix: stopped the dev server, deleted the `.next` folder (`Remove-Item -Recurse -Force .next` — note this is PowerShell syntax, not `rm -rf`), restarted `npm run dev`. Verified by: clean redirect to `/sign-in` with no 500, dashboard rendering correctly for a signed-in user.

...
- Phase: **0 — Foundation** (in progress — steps 0.1–0.12 of 0.15 done, see plan in chat history)
  ...
  - Next up: 0.13 tRPC setup (one trivial `hello` procedure wired to the dashboard). **Read Day 5 lesson before starting.** Paused here deliberately at a clean, fully-synced stopping point rather than starting 0.13 (a bigger step, comparable to 0.9 Clerk) at the end of a long session — see chat history.
- Last green-light check: none yet (`npx prisma migrate status` not yet re-verified post-merge). Currently green: `npm run typecheck`, `npm run lint`, `npm run test`, `npm run test:e2e` (now covers all 3 Phase 0 DoD e2e requirements), `npm run build`.
- Open blockers:
  - **Next.js 16 `middleware.ts` → `proxy.ts` rename** — reconfirmed again in step 0.12's `npm run build` output ("The 'middleware' file convention is deprecated"). Still not investigated. `middleware.ts` still works, low urgency, but should be researched before Phase 0 closes out (step 0.15).
  - **Recurring `.next` cache EPERM errors on Windows** — happened again in step 0.12 (previously step 0.11), always as noise from the Playwright-launched dev server, never actually failing a test so far. Root cause still not pinned down (confirmed not OneDrive; likely antivirus real-time scanning or similar). Worth a permanent fix (e.g. excluding the project folder from AV scanning) before it causes a real failure, but not blocking.
- Last commit note: `test: add the three Phase 0 DoD e2e tests via Clerk testing tokens` (PR #16, squash-merged by Rad)

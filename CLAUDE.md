...
- Last green-light check: **first real one, via CI on PR #25**: typecheck/lint/unit-tests/build/e2e-tests all genuinely green on a clean machine (not just locally). `npx prisma migrate status` still not covered by CI (see step 0.14 known simplification) — only checked locally when Rad ran migrations directly.
- Open blockers:
...
- Last commit note: `docs: update CURRENT STATE with Phase 0 progress (step 0.14)` (PR #24, squash-merged) + follow-up `fix: run next typegen before typecheck in CI` / `fix: exclude e2e/ from Vitest and switch to native tsconfig-paths resolution` (PR #25, squash-merged)

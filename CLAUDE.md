...
- Phase: **0 — Foundation** (in progress — steps 0.1–0.11 of 0.15 done, see plan in chat history)
  ...
  - Next up: 0.12 write the three Phase 0 DoD e2e tests (logged-out→/login, user→/dashboard, non-admin blocked from /admin).
...
- Open blockers: **Next.js 16 `middleware.ts` → `proxy.ts` rename** — Next printed a codemod suggestion (`npx @next/codemod@canary middleware-to-proxy .`) during step 0.11 testing; not yet investigated. `middleware.ts` still works today, low urgency, but should be researched before Phase 0 closes out (step 0.15) in case it's a hard requirement rather than cosmetic.
- Last commit note: `feat: add empty dashboard route gated behind login` (PR #14, squash-merged by Rad)

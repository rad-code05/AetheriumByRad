...
- Phase: **0 — Foundation** (in progress — steps 0.1–0.8 of 0.15 done, see plan in chat history)
  ...
  - Next up: 0.9 Clerk integration (sign-up/sign-in + middleware).
- Last green-light check: none yet (`npx prisma migrate status` not yet re-verified post-merge). Currently green: `npm run typecheck`, `npm run lint`, `npm run test`, `npm run test:e2e`, `npm run build`.
- Open blockers: none
- Last commit note: `feat: add Prisma + Neon Postgres, minimal User model` (PR #8, squash-merged by Rad)

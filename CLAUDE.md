...
- Phase: **0 — Foundation** (in progress — steps 0.1–0.13 of 0.15 done, see plan in chat history)
  ...
  - Next up: 0.14 GitHub Actions CI (lint/typecheck/tests on every PR).
- Last green-light check: none yet (`npx prisma migrate status` not yet re-verified post-merge). Currently green: `npm run typecheck`, `npm run lint`, `npm run test`, `npm run test:e2e` (covers all 3 Phase 0 DoD e2e requirements), `npm run build`.
- Open blockers:
  - **Next.js 16 `middleware.ts` → `proxy.ts` rename** — reconfirmed again in step 0.13's `npm run build` output. Still not investigated. `middleware.ts` still works, low urgency, but should be researched before Phase 0 closes out (step 0.15).
  - **Recurring `.next` cache EPERM errors on Windows** — seen in steps 0.11 and 0.12, always as noise from a Playwright-launched dev server, never actually failing a test so far. Root cause still not pinned down (confirmed not OneDrive; likely antivirus real-time scanning or similar). Worth a permanent fix before it causes a real failure, but not blocking.
- Last commit note: `feat: add tRPC setup with hello procedure wired to dashboard` (PR #19, squash-merged)
